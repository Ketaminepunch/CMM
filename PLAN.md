...
**Prompting strategy (`src/prompts.py`) carries the semantic load.** With the grammar this simple, extraction accuracy comes from prompts: a function-selection prompt listing `name: description` pairs with few-shot examples, then **one dedicated few-shot prompt per parameter** (per type: number / string / boolean) rather than asking for all arguments at once — this measurably improves extraction for functions with several similar parameters.
...
│   ├── prompts.py           # function-selection prompt + one few-shot template per parameter type
...
│   ├── decoder.py           # generation loop: forced literals + masked-argmax at decision points
│   └── pipeline.py          # orchestrates: load -> per-prompt generate (per-item try/except) -> write output
├── data/
│   └── input/
...
5. **Decoder loop** (`decoder.py`): wire `Small_LLM_Model` + grammar; forced-literal fast path (encode + append, no forward pass); masked-argmax at decision points (numpy `-inf` mask); hard max-token safety cap with a clear internal error (caught upstream, never crashes the run).
6. **Prompts + pipeline + CLI**: function-selection prompt, per-parameter few-shot templates per type; instantiate the model once; per-prompt try/except; assemble `{prompt, name, parameters}` and write `function_calling_results.json` via pydantic model.
